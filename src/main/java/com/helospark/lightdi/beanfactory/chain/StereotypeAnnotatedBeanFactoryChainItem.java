...
public class StereotypeAnnotatedBeanFactoryChainItem implements BeanFactoryChainItem {
    ...
    public List<DependencyDescriptor> extractDependencies(DependencyDescriptor dependencyToCreate) {
        ...
        result.addAll(injectDescriptorsToDependencies
                .extract(stereotypeDependency.getConstructorDescriptor().stream()
                        .map(a -> a.getDependencyDescriptor())));
        result.addAll(
                injectDescriptorsToDependencies.extract(
                        stereotypeDependency.getFieldDescriptor().stream().map(a -> a.getInjectionDescriptor())));
        result.addAll(injectDescriptorsToDependencies
                .extract(stereotypeDependency.getSetterDescriptor().stream()
                        .flatMap(a -> a.getInjectionDescriptor().stream())));
        return result;
    }
}
