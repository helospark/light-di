...
Since Android does not support runtime classpath scanning, make sure to not disable annotation preprocessor during compile time (ex. do NOT add `-proc:none`).
While annotation processing is a standard Java feature, IDEs may not support it by default.
(For Eclipse you can enable it, by installing `m2e-apt` plugin, for Android Studio (IDEA) you can enable by `Settings > Build, Execution, Deployment > Compiler > Annotation Processors > Enable annotation processing`.)
Also you need to define the jar as a annotationProcessor explicitly in your build.gradle file. You file should look like:
